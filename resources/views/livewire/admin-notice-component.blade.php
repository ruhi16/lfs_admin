<div class="max-w-7xl spacey mx-auto sm:px-6 lg:px-8 flex flex-col bg-slate-500">
    ...
    <div class="max-w-6xl spacey mx-auto sm:px-6 lg:px-8 ">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 table-auto">
            <thead>
                <tr>
                    <th colspan="5"
                        class="p-5 text-lg font-semibold text-center bg-slate-300 text-gray-900 dark:text-white">All
                        Notices</th>
                    <th colspan="3" class="p-5 text-lg font-semibold text-center bg-slate-300 text-gray-900 dark:text-white">
                        <button class="bg-blue-500 text-white px-4 py-2 rounded" wire:click="edit(0)">Add New</button>
                    </th>
                </tr>
            </thead>
            {{-- <caption class="p-5 text-lg font-semibold text-left text-gray-900 dark:text-white">All Notices
            </caption> --}}
                ...
                @foreach($notices as $notice)
                    <tr
                        ...
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 dark:text-white dark:bg-gray-800">
                            {{ $notice->title }}</th>
                        <th scope="row"
                            class="px-6 py-4 font-medium text-gray-900 dark:text-white dark:bg-gray-800 text-ellipsis "
                            width="5%">{{ $notice->desc }}</th>
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 dark:text-white dark:bg-gray-800">
                            {{ $notice->desc }}</th>
                        <th scope="row"
                            class="px-6 py-4 font-medium text-gray-900 dark:text-white dark:bg-gray-800 text-ellipsis"
                            width="15%">{{ $notice->dop }} to {{ $notice->doe }}</th>
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 dark:text-white dark:bg-gray-800">
                            {{-- {{ $notice->fileaddr }}:<br /> --}}
    ...
    <div>
        ...
        {{-- <button class="bg-blue-500 text-white px-4 py-2 rounded" wire:click="edit({{ $notice_id }})">Open
            Modal</button> --}}
        {{-- <button class="bg-blue-500 text-white px-4 py-2 rounded" onclick="openModal()">Open Modal</button> --}}

        <!-- Modal -->
        <div id="modal"
            class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 {{ $modal_is_open ? '' : 'hidden' }}">
            <div class="bg-white rounded-lg shadow-lg p-6 w-1/2">
                ...
                <form wire:submit.prevent="saveNotice" class="max-lg mx-auto" enctype="multipart/form-data">
                    ...
                    {{-- {{ json_encode($notice_selected) }} --}}
                        ...
                        <div class="mb-4">
                            <label for="dop" class="block text-gray-700 text-sm font-bold mb-2">Date of
                                Publication:</label>
                            <input type="date" wire:model="dop" id="dop"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            @error('dop') <span class="text-red-500 text-xs italic">{{ $message }}</span> @enderror
                        </div>

                        <div class="mb-4">
                            <label for="doe" class="block text-gray-700 text-sm font-bold mb-2">Date of
                                Expiration:</label>
                            <input type="date" wire:model="doe" id="doe"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            @error('doe') <span class="text-red-500 text-xs italic">{{ $message }}</span> @enderror
                        </div>

                        {{-- <div class="mb-4">
                            <label for="fileaddr" class="block text-gray-700 text-sm font-bold mb-2">File
                                Address:</label>
                            <input type="text" wire:model="fileaddr" id="fileaddr"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="file" wire:model="fileaddr" id="fileaddr" accept="image/*"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            @error('fileaddr') <span class="text-red-500 text-xs italic">{{ $message }}</span> @enderror

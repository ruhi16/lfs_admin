<x-app-layout>
    {{-- <x-slot name="header"> --}}
        ...
        <h2 class="font-semibold text-2xl text-gray-800 leading-tight">
            {{-- {{ 'Dashboard: ' . __(auth()->user()->role->name) . ': ' . __(auth()->user()->teacher->id).'-' .
            __(auth()->user()->teacher->name).' (User Name: '.__(auth()->user()->name).')' }} --}}
            {{-- {{ 'Dashboard: ' . __('Role-').__(auth()->user()->role->description) . ': ' .
            __(auth()->user()->teacher->id).'-' . __(auth()->user()->teacher->name) }} --}}
        ...
        <table class="w-4xl mx-auto text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr>
                    ...
                    <td class="px-6 py-4 dark:text-white">
                        <a href="{{ route('ws.caraosel-view') }}"
                            class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Goto</a>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 dark:text-white">Welcome Screen</td>
                    <td class="px-6 py-4 dark:text-white">Gallery</td>
                    <td class="px-6 py-4 dark:text-white">
                        <ul>
                            <li onclick="navigateToRoute()">Menu Item</li>
                            <li onclick="window.location.href = '{{ route('idcard') }}'">Menu Item</li>
                        </ul>
                    </td>
                </tr>
